...
Tree::Tree(const Tree & tree) : anker(nullptr), l_node_id(-1)
{
	std::vector<TreeNode *> result;
	getNodes(tree.anker, result);
	for (std::vector<TreeNode *>::iterator it(result.begin()), end(result.end()); it != end; ++it)
		addNode((*it)->getName(), (*it)->getAlter(), (*it)->getEinkommen(), (*it)->getPLZ());
}
